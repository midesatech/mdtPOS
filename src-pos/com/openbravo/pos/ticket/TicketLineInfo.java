...
public class TicketLineInfo implements SerializableWrite, SerializableRead, Serializable {
    ...
    private TaxInfo tax;
    private TaxInfo taxConIva; /* JDP001 */
    ...
    private boolean m_bIsArtEsp;
    
    /* 16/08/2017 JDP */
    private boolean m_bNoMosIva;
    
    

    /** Creates new TicketLineInfo */   
    public TicketLineInfo(TicketProductInfo product, double dMultiply, double dPrice, TaxInfo tax, Properties attributes, TaxInfo taxConIva) {
        this.product = product; 
        m_dMultiply = dMultiply;
        m_dPrice = dPrice;
        this.tax = tax;
        this.taxConIva = taxConIva;
        this.attributes = attributes;

        m_sTicket = null;
        m_iLine = -1;
        m_bNoMosIva = product.noMosIva();
    }
     
    public TicketLineInfo(TicketProductInfo product, double dMultiply, double dPrice, TaxInfo tax, TaxInfo taxConIva) {
        this(product, dMultiply, dPrice, tax, new Properties(), taxConIva);
    }    
    public TicketLineInfo() {
        this(new TicketProductInfo(), 0.0, 0.0, null, new Properties(), null);
    }

    public TicketLineInfo(ProductInfoExt oProduct, double dMultiply, double dPrice, TaxInfo tax, Properties attributes, TaxInfo taxConIva) {
        if (oProduct == null) {
        ...
        m_bIsArtEsp = product.isArtEsp();
        m_bNoMosIva = product.noMosIva();
    }    
    public TicketLineInfo(ProductInfoExt oProduct, double dPrice, TaxInfo tax, Properties attributes, TaxInfo taxConIva) {       
        this(oProduct, 1.0, dPrice, tax, attributes, taxConIva);
    }
    ...
    public TicketLineInfo(TicketLineInfo line) {
        ...
        tax = line.tax;
        taxConIva = line.taxConIva;
        attributes = line.attributes;
        m_sTicket = null;
        m_iLine = -1;
        m_bIsArtEsp = line.isArtEsp();
        m_bNoMosIva = line.noMosIva();
    }
    ...
    public void writeValues(DataWrite dp) throws BasicException {
        ...
        dp.setBoolean(11,product.isArtEsp());  /*18/12/2014 JDP*/
        dp.setBoolean(12,product.noMosIva());  /* JDP002 */
    }
    ...
    public void readValues(DataRead dr) throws BasicException {
        ...
        boolean prodartesp = dr.getBoolean(12).booleanValue();
        boolean prodnomosiva = dr.getBoolean(13).booleanValue();
        
        product = new TicketProductInfo(prodid, prodname, prodcom, prodartesp, prodnomosiva);
    }
    ...
    public TicketLineInfo copyTicketLine() {
        ...
        l.tax = tax;   
        l.taxConIva = taxConIva;
        l.attributes = attributes;        
        l.product = product.copyTicketProduct();
        l.m_bIsArtEsp = m_bIsArtEsp;   /*18/12/2014 JDP*/
        l.m_bNoMosIva = m_bNoMosIva;   /* JDP002 */
        return l;
    }
    ...
    public void SetArtEsp(Boolean x)
    {
    	this.m_bIsArtEsp=x;
    }
    ...
    public boolean isArtEsp() {
        return product.isArtEsp();       
    }
    
    public void setProductArtEsp(Boolean a)
    {
    	product.setArtEsp(a);
    }
    ...
    public void SetTicketPadre(String id)
    {
    	m_sTicket=id;
    }
    ...
    public String printTax() {
    	...
    	try {
    		retorno =  Formats.CURRENCY.formatValue(new Double(getTax()));
    	}catch(Exception ex) {
    ...
    /* JDP001 */
    public TaxInfo getTaxInfoConIva() {
        return taxConIva;
    } 
    
    /* JDP002 */
    public void setTaxInfoConIva(TaxInfo value) {
        taxConIva = value;
    } 
    
    public String printTaxConIva() {
        return Formats.CURRENCY.formatValue(new Double(getTaxConIva()));
    }
    
    public double getTaxRateConIva() {
        return taxConIva == null ? 0.0 : taxConIva.getRate();
    }
    
    public double getTaxConIva() {
        return m_dMultiply * m_dPrice * getTaxRateConIva();
    }
    
    public double getValueConIva() {
        return m_dMultiply * m_dPrice * (1.0 + getTaxRateConIva());
    }
    
    public String printValueConIva() {
        return Formats.CURRENCY.formatValue(new Double(getValueConIva()));
    }
    
    
    /* JDP002 */
    public boolean noMosIva() {
        return product.noMosIva();       
    }
    
    public void setProductNoMosIva(Boolean a)
    {
    	product.setNoMosIva(a);
    }
}
