...
public class TicketInfo implements SerializableRead, Externalizable {
	...
	private Boolean m_IsDian;
	private Double m_dMaxDebt = 0.0;
	private Double m_dCupArtEsp = 0.0;
	private int m_iCopiaTicket = 0;
	...
	public void writeExternal(ObjectOutput out) throws IOException {
		// esto es solo para serializar tickets que no estan en la bolsa de
		// tickets pendientes
	...
	public void readValues(DataRead dr) throws BasicException {
		...
		m_Customer = customerid == null ? null : new CustomerInfo(dr.getString(7), dr.getString(8), dr.getString(9));
	...
	public void setCopiaTicket(int valor) {
		m_iCopiaTicket = valor;
	}

	public int getCopiaTicket() {
		return m_iCopiaTicket;
	}

	public void SetIsDian(Boolean valor) {
		m_IsDian = valor;
	}

	public void setMaxDebt(Double a) {
		this.m_dMaxDebt = a;
	}

	public void setCupArtEsp(Double a) {
		this.m_dCupArtEsp = a;
	}

	public String printMaxDebt() {
		return Formats.DOUBLE.formatValue(new Double(m_dMaxDebt));
	}

	public String printCupArtEsp() {
		return Formats.DOUBLE.formatValue(new Double(m_dCupArtEsp));
	}
	...
	public double getSubTotal() {
		...
		if (m_GlobalDiscountInfo != null && m_GlobalDiscountInfo.isPercentage()) {
			...
			for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
				oLine = i.next();
				dSuma += oLine.getSubValue()
						- (oLine.getMultiply() / count * m_GlobalDiscountInfo.getQuantity() / (1 + oLine.getTaxRate()));
			}
		}
		return dSuma;
	}
	...
	public double getTax() {
		...
		if (m_GlobalDiscountInfo != null && m_GlobalDiscountInfo.isPercentage()) {
			...
			for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
				oLine = i.next();
				dSuma += oLine.getTax() - (oLine.getMultiply() / count * m_GlobalDiscountInfo.getQuantity()
						* oLine.getTaxRate() / (1 + oLine.getTaxRate()));
			}
		}
		return dSuma;
	}
	...
	public void setLines(List<TicketLineInfo> l) {
		List<TicketLineInfo> temp = new ArrayList<TicketLineInfo>();
		for(TicketLineInfo item: l) {
			item.setProductNoMosIva(item.getProduct().noMosIva());
		    temp.add(item);
		}
		m_aLines = temp;
	...
	public void SetTicketIdLines() {
		List<TicketLineInfo> listAux = new ArrayList();
		for (TicketLineInfo inf : m_aLines) {
			inf.SetTicketPadre(m_sId);
			listAux.add(inf);
		}
		m_aLines = listAux;
	}
	...
	public String printCustomer() {
		return m_Customer == null ? "" : StringUtils.encodeXML(m_Customer.getName());
	}
	...
	public String printTotalConIva() {
		return Formats.CURRENCY.formatValue(new Double(getTotalConIva()));
	}
	...
	public double getTaxConIva() {
		...
		if (m_GlobalDiscountInfo != null && m_GlobalDiscountInfo.isPercentage()) {
			...
			for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
				oLine = i.next();
				dSuma += oLine.getTaxConIva() - (oLine.getMultiply() / count * m_GlobalDiscountInfo.getQuantity()
						* oLine.getTaxRateConIva() / (1 + oLine.getTaxRateConIva()));
			}
		}
		return dSuma;
	}

	public TicketTaxInfo[] getTaxLinesConIva() {
		...
		for (Iterator<TicketLineInfo> i = m_aLines.iterator(); i.hasNext();) {
			...
			TicketTaxInfo t = m.get(oLine.getTaxInfoConIva());

			if (t == null) {
				t = new TicketTaxInfo(oLine.getTaxInfoConIva());
				m.put(t.getTaxInfo(), t);
			}
			t.add(oLine.getSubValue());
		}
