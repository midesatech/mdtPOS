...
/* JOSE DE PAZ -JDP-
 * 05/12/2014 Generación de versión 1.0
 *            Se modifica para elegir entre dos reportes
 *            Reporte X o Reporte Z
 * 
 * JOSE DE PAZ -JDP-
 * 11/06/2016 Generar reporte de cierre de caja por consecutivo
 *            el usuario ingresa el número de consecutivo. 
 */
...
/* 20/06/2014 Consecutivo cierre de caja */
...
public class JPanelCloseMoney extends JPanel implements JPanelView, BeanFactoryApp {
	...
	private DataLogicSales m_dlSales; /* 20/06/2014 */
	...
	public void init(AppView app) throws BeanFactoryException {
		...
		/* Modificación por problemas de cierre de caja, 29/10/2015 */
		m_dlSales = (DataLogicSales) m_App.getBean(
				"com.openbravo.pos.forms.DataLogicSalesCreate"); /*
																	 * 20/06/
																	 * 2014
																	 */
		...
		m_jsalestable.setDefaultRenderer(Object.class,
				new TableRendererBasic(new Formats[] { Formats.STRING, Formats.CURRENCY, Formats.CURRENCY }));
	...
	/* 11/06/2016 */
	private void printPayments(PaymentsModel aux) {
	...
	private void initComponents() {
		...
		m_jReporteX = new javax.swing.JRadioButton("Reporte X");
		m_jReporteZ = new javax.swing.JRadioButton("Reporte ZETA");
		m_jPanelRepZNum = new javax.swing.JPanel();
		/* 11/06/2016 */
		m_jLblNumRepZ = new javax.swing.JLabel("No.:");
		m_jNumRepZ = new javax.swing.JTextField();
		m_jGenRepZ = new javax.swing.JButton("Imprimir");
		...
		grupo.add(m_jReporteZ);
		grupo.add(m_jReporteX);

		javax.swing.JPanel radioPanel = new javax.swing.JPanel(new GridLayout(0, 2));
		radioPanel.add(m_jReporteX);
		radioPanel.add(m_jReporteZ);
		add(radioPanel);
		radioPanel.setBounds(400, 36, 220, 37);
		m_jReporteX.setSelected(true);
		/* 05/12/2014 JDP */

		/* 11/06/2016 */
		m_jPanelRepZNum.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("Reporte ZETA"),
				BorderFactory.createEmptyBorder(2, 2, 2, 2)));
		m_jPanelRepZNum.add(m_jLblNumRepZ);
		m_jNumRepZ.setColumns(10);
		m_jNumRepZ.setPreferredSize(new Dimension(20, 23));
		m_jNumRepZ.setSize(new Dimension(20, 23));
		m_jPanelRepZNum.add(m_jNumRepZ);
		m_jPanelRepZNum.add(m_jGenRepZ);
		m_jGenRepZ.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				m_jGenRepZActionPerformed(evt);
			}
		});
		add(m_jPanelRepZNum);
		m_jPanelRepZNum.setBounds(400, 69, 270, 58);
	...
	private void m_jCloseCashActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_m_jCloseCashActionPerformed
		...
		/* 05/12/2014 JDP */
		Date dNow = new Date();
		if (m_jReporteX.isSelected()) {
			m_PaymentsToClose.setTipoCierre(AppLocal.getIntString("Label.TipoCierreX"));
			m_PaymentsToClose.setDateEnd(dNow);
			printPayments();
			
			/* Registrar en base de datos envío de reporte */
			...
			if (res == JOptionPane.YES_OPTION) {
				...
				int iConsecutivo = 0; /* 20/06/2014 */
				try {
					// Cerramos la caja si esta pendiente de cerrar.
					if (m_App.getActiveCashDateEnd() == null) {
						/* 20/06/2014 Consecutivo cierre de caja */
						iConsecutivo = m_dlSales.getNextClosedCashIndex().intValue();
						new StaticSentence(m_App.getSession(),
								"UPDATE CLOSEDCASH SET DATEEND = ?, CLOSEDID=? WHERE HOST = ? AND MONEY = ?",
								new SerializerWriteBasic(
										new Datas[] { Datas.TIMESTAMP, Datas.INT, Datas.STRING, Datas.STRING }))
												.exec(new Object[] { dNow, iConsecutivo,
														m_App.getProperties().getHost(), m_App.getActiveCashIndex() });
					}
				...
				try {
					...
					m_dlSystem
							.execInsertCash(new Object[] { m_App.getActiveCashIndex(), m_App.getProperties().getHost(),
									m_App.getActiveCashDateStart(), m_App.getActiveCashDateEnd() });

					// ponemos la fecha de fin
					m_PaymentsToClose.setDateEnd(dNow);
					m_PaymentsToClose
							.setConsecutivo(iConsecutivo);/*
															 * 20/06/2014
															 */
					m_PaymentsToClose
							.setTipoCierre(AppLocal.getIntString("Label.TipoCierreZ") + " - No. " + iConsecutivo);
	...
	/* 11/06/2016 */
	private void m_jGenRepZActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_m_jCloseCashActionPerformed
		int iConsecutivo = 0;
		// LoadData
		PaymentsModel m_PaymentsToCloseAux = null;
		try {
			iConsecutivo = Integer.parseInt(this.m_jNumRepZ.getText().trim());

			Object[] cierreHistorico = m_dlSystem.finHistoricCash(iConsecutivo);

			if (cierreHistorico != null) {
				com.openbravo.data.loader.EntidadApp appViewAux = new com.openbravo.data.loader.EntidadApp();
				appViewAux.setActiveCashDateStart((Date)cierreHistorico[2]);
				appViewAux.setActiveCashDateEnd((Date)cierreHistorico[3]);
				appViewAux.setSession(m_App.getSession());
				appViewAux.setHost((String)cierreHistorico[1]);
				appViewAux.setActiveCashIndex((String)cierreHistorico[0]);
				
		
				m_PaymentsToCloseAux = PaymentsModel.loadInstance(appViewAux, iConsecutivo);
				m_PaymentsToCloseAux.setConsecutivo(iConsecutivo);
				m_PaymentsToCloseAux
						.setTipoCierre(AppLocal.getIntString("Label.TipoCierreZ") + " - No. " + iConsecutivo);
				printPayments(m_PaymentsToCloseAux);
			}
			else
			{
				MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, "***  No. de Cierre de Caja no existe.  ***");
				msg.show(this);
			}
	...
	private javax.swing.JRadioButton m_jReporteX; // 05/12/2014 JDP
	private javax.swing.JRadioButton m_jReporteZ; // 05/12/2014 JDP
	private javax.swing.JPanel m_jPanelRepZNum;
	private javax.swing.JLabel m_jLblNumRepZ;     /* 11/06/2016 */
	private javax.swing.JTextField m_jNumRepZ;
	private javax.swing.JButton m_jGenRepZ;
