...
// 24-07-2017  JDP001  Cambio en alerta cuando CLIENTE no tiene CUPO suficiente
...
/*
 * 16-09-2014 JOSE DE PAZ JPD REQ140914 Se realizan validaciones para pagar con
 * lo que sobra del cr�dito �nicamente 07/07/2016 JOSE DE PAZ JPD1 Forma de
 * Pago: Venta Cr�dito
 * 
 * 
 */
...
public abstract class JPaymentSelect extends javax.swing.JDialog implements JPaymentNotifier {
	...
	/* jdp */
	private Boolean m_FDian;

	/* 18/12/2014 JDP */
	private double m_dSumArtEsp;
	...
	public Boolean getFDian() {
		// return m_jFDian.isSelected();
		return m_FDian;
	}

	public void SetFDian(Boolean valor) {
		// m_jFDian.setSelected(valor);
		m_FDian = valor;
	}

	public Double getSumArtEsp() {
		return m_dSumArtEsp;
	}

	public boolean showDialog(double total, CustomerInfo customer, double sumArtEsp, int formaPago) {

		m_aPaymentInfo = new PaymentInfoList();
		accepted = false;

		m_dTotal = total;
		m_dSumArtEsp = sumArtEsp; /* 18/12/2014 JDP */
	...
	/* 10/12/2014 */
	public class JPaymentPostPagoDescNomCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentPostPagoDescNom(JPaymentSelect.this);
		}

		public String getKey() {
			return "payment.postpagodesnom";
		}

		public String getLabelKey() {
			return "tab.postpagodesnom";
		}

		public String getIconKey() {
			return "/com/openbravo/images/prepaiddn.png";
		}
	}

	public class JPaymentPrepagoSobreCupoCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentPrepagoSobreCupo(JPaymentSelect.this);
		}

		public String getKey() {
			return "payment.prepagosobrecupo";
		}

		public String getLabelKey() {
			return "tab.prepagosobrecupo";
		}

		public String getIconKey() {
			return "/com/openbravo/images/prepaidsc.png";
		}
	}

	public class JPaymentPrepagoCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentPrepago(JPaymentSelect.this);
		}

		public String getKey() {
			return "payment.prepago";
		}
		...
		public String getIconKey() {
			return "/com/openbravo/images/prepaid.png";
		}
	}

	public class JPaymentPrepagoRefundCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentPrepago(JPaymentSelect.this);
		}
	...
	public class JPaymentPostPagoDescNomRefundCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentPostPagoDescNom(JPaymentSelect.this);
		}
	...
	public class JPaymentPrepagoSobreCupoRefundCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentPrepagoSobreCupo(JPaymentSelect.this);
		}
	...
	public class JPaymentValeCompassCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentValeCompass(JPaymentSelect.this);
		}

		public String getKey() {
			return "payment.valecompass";
		}

		public String getLabelKey() {
			return "tab.valecompass";
		}
	...
	public class JPaymentValeCompassRefundCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentValeCompass(JPaymentSelect.this);
		}
	...
	/* 07/07/2016 JDP1 */
	public class JPaymentVentaCreditoCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentVentaCredito(JPaymentSelect.this);
		}

		public String getKey() {
			return "payment.ventacredito";
		}

		public String getLabelKey() {
			return "tab.ventacredito";
		}

		public String getIconKey() {
			return "/com/openbravo/images/ventascredito.png";
		}
	}

	public class JPaymentVentaCreditoRefundCreator implements JPaymentCreator {
		public JPaymentInterface createJPayment() {
			return new JPaymentVentaCredito(JPaymentSelect.this);
		}
	...
	private void printState(int fp) {
		...
		if (this.customerext != null) {
			if (this.customerext.isSobreCupo()) {
				if (m_jTabPayment.getTabCount() == 11) {
					if (fp >= 0) {
						m_jTabPayment.setSelectedIndex(fp); // selecciono
															// prepagosobrecupo
					} else {
						if (this.m_dSumArtEsp > 0)
							if (m_jTabPayment.getTabCount()>=8)
							    m_jTabPayment.setSelectedIndex(8); // selecciono
																// prepagosobrecupo
						else
							m_jTabPayment.setEnabledAt(8, false);
					}
				} else 
					if (this.m_dSumArtEsp > 0)
						if (m_jTabPayment.getTabCount()>=7)
					        m_jTabPayment.setSelectedIndex(6);
				else
					System.out.println(m_jTabPayment.getTabCount());
					if (m_jTabPayment.getTabCount()>=7)
					m_jTabPayment.setEnabledAt(6, false);

			} else {
				if (fp >= 0) {
					m_jTabPayment.setSelectedIndex(fp);
				} else {
	...
	private void initComponents() {
		...
		m_jFDian = new javax.swing.JCheckBox();
		jPanel7 = new javax.swing.JPanel();
		m_jLblFDian = new javax.swing.JLabel();
		...
		m_jLblFDian.setText("�Afecta numeraci�n DIAN?"); // NOI18N
		m_jLblFDian.setFont(new java.awt.Font("Dialog", 1, 16));
		jPanel7.add(m_jLblFDian);

		// m_jFDian.setBackground(java.awt.Color.white);
		m_jFDian.setFont(new java.awt.Font("Dialog", 1, 14));
		m_jFDian.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
		m_jFDian.setBorder(javax.swing.BorderFactory.createCompoundBorder(
				javax.swing.BorderFactory
						.createLineBorder(javax.swing.UIManager.getDefaults().getColor("Button.darkShadow")),
				javax.swing.BorderFactory.createEmptyBorder(1, 4, 1, 4)));
		// m_jFDian.setOpaque(true);
		m_jFDian.setPreferredSize(new java.awt.Dimension(205, 25));
		m_jFDian.setRequestFocusEnabled(false);
		jPanel7.add(m_jFDian);
		...
		jPanel3.setLayout(new java.awt.BorderLayout());
		// jPanel3.add(m_jLblFDian, java.awt.BorderLayout.LINE_END);
		...
		setBounds((screenSize.width - 772) / 2, (screenSize.height - 577) / 2, 772, 577);// JDP
	...
	private void m_jButtonAddActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_m_jButtonAddActionPerformed
		...
		if (returnPayment != null) {
			/* 16-09-2014 */
			double dSaldoCupo = 0.0;
			dSaldoCupo = ((JPaymentInterface) m_jTabPayment.getSelectedComponent()).getSaldoCupo();
			if ((returnPayment.getName() == "debt") || (returnPayment.getName() == "prepago")) {
				if (dSaldoCupo == 0) {
					JOptionPane.showMessageDialog(this, AppLocal.getIntString("message.cannotcupoa"), "Cupo Cliente",
							JOptionPane.PLAIN_MESSAGE);
					return;
				} else {
					VerificaRetornoSaldoCupo(returnPayment, dSaldoCupo, 0);
				}
			} else {
	...
	private double VerificaCupos(double dSaldoCupo) {
		double ret = 0.0;
		LinkedList<PaymentInfo> listDebt = m_aPaymentInfo.getPayments();
		Iterator<PaymentInfo> i = listDebt.iterator();
		double totalDebt = 0.0;
		while (i.hasNext()) {
			PaymentInfo p = (PaymentInfo) i.next();
			if ((p.getName() == "debt") || (p.getName() == "prepago")) {
				totalDebt = totalDebt + p.getTotal();
			}
		}
		if (totalDebt > 0) {
			ret = dSaldoCupo - totalDebt;
			if (ret < 0)
				ret = -1;
			else if (ret > 0)
				ret = 1;
		} else {
			Boolean bAlcanza = ((JPaymentInterface) m_jTabPayment.getSelectedComponent()).getAlcanza();
			if (bAlcanza)
				ret = 1;
			else {
				ret = -1;
	...
	private void VerificaRetornoSaldoCupo(PaymentInfo returnPayment, double dSaldoCupo, int flag) {
		double dVerificaCupos = VerificaCupos(dSaldoCupo);
		int ret = 0;
		if (dVerificaCupos < 0) {
			
			// JDP001
			JOptionPane.showMessageDialog(this,
					AppLocal.getIntString("message.cannotcupo"),
					"Cupo Cliente", JOptionPane.ERROR_MESSAGE);
			return;
            /*
			if (ret == 0) {
				if (returnPayment.getName() == "prepago") {
					returnPayment = new PaymentInfoTicket(dSaldoCupo, "prepago");
				} else {
					returnPayment = new PaymentInfoTicket(dSaldoCupo, "debt");
				}
			...
			*/
			...
			if (dVerificaCupos > 0) {
				...
				JOptionPane.showMessageDialog(this, AppLocal.getIntString("message.cannotaddcupo"), "Cupo Cliente",
						JOptionPane.PLAIN_MESSAGE);
			}
		}
	}
	...
	private void m_jTabPaymentStateChanged(javax.swing.event.ChangeEvent evt) {// GEN-FIRST:event_m_jTabPaymentStateChanged
		...
		if (m_jTabPayment.getSelectedComponent() != null) {
			...
			/* 18/12/2014 JDP */
			if ((m_jTabPayment.getSelectedIndex() == 9) || (m_jTabPayment.getSelectedIndex() == 10) || (m_jTabPayment.getTabCount() == 11)) {
				//System.out.println("pago 1 " + m_jTabPayment.getSelectedIndex());
				if ((m_jTabPayment.getSelectedIndex() == 9))
				{
				...
				PaymentInfo returnPayment = ((JPaymentInterface) m_jTabPayment.getSelectedComponent()).executePayment();
				//System.out.println("returnPayment.getName() 1 " + returnPayment.getName());
				if ((returnPayment.getName().equals("prepago"))) {
					//System.out.println("this.SetFDian(false)");
					this.SetFDian(false);
				}
						
//				if ((m_jTabPayment.getSelectedIndex() == 6) || (m_jTabPayment.getSelectedIndex() == 7)
//						|| (m_jTabPayment.getSelectedIndex() == 9) 
//						) {
//					this.SetFDian(false);
//				}
//			} else {
//				if ((m_jTabPayment.getSelectedIndex() == 4) || (m_jTabPayment.getSelectedIndex() == 5) ) {
//					this.SetFDian(false);
//				}
			}
			else
			{
				//System.out.println("pago 0 " + m_jTabPayment.getSelectedIndex());
				
				PaymentInfo returnPayment = ((JPaymentInterface) m_jTabPayment.getSelectedComponent()).executePayment();
				//System.out.println("returnPayment.getName() " + returnPayment.getName());
				if ((returnPayment.getName().equals("prepago"))) {
					//System.out.println("this.SetFDian(false)");
					this.SetFDian(false);
				}
			...
			// switch(m_jTabPayment.getSelectedIndex())
			// {
			// case 0:
			// this.SetFDian(true); /*efectivo*/
			// break;
			// case 1:
			// this.SetFDian(false);
			// break;
			...
			// break;
			// case 3:
			// this.SetFDian(true); /*tarjeta credito o debito*/
			// break;
			// case 4:
			// this.SetFDian(false);
			// break;
	...
	private void m_jButtonOKActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_m_jButtonOKActionPerformed
        //System.out.println("boton m_jButtonOKActionPerformed");
		boolean flag = false;
		PaymentInfo returnPayment = ((JPaymentInterface) m_jTabPayment.getSelectedComponent()).executePayment();
		if (returnPayment != null) {
			/* 16-09-2014 */
			...
			if ((returnPayment.getName() == "debt") || (returnPayment.getName() == "prepago")) {
				...
				if (dSaldoCupo == 0) {
					/* 18/12/2014 JDP */
					if (this.m_dSumArtEsp > 0) {
						// validar que alcance el cupo articulo especial
						if (this.customerext.getCupArtEsp() >= this.m_dSumArtEsp) {
							// si hay cupo para articulo especial

							if ((returnPayment.getTotal() - this.m_dSumArtEsp) > 0) {
								// hay mas productos que no son articulos
								// especiales
								JOptionPane.showMessageDialog(this,
										AppLocal.getIntString("message.cannotsobrecupoc") + "\n\n"
												+ AppLocal.getIntString("message.cannotsobrecupod"),
										AppLocal.getIntString("message.cannotsobrecupoe"), JOptionPane.PLAIN_MESSAGE);
								return;
							} else {
								m_aPaymentInfo.add(returnPayment);
								accepted = true;
							...
							JOptionPane.showMessageDialog(this, AppLocal.getIntString("message.cannotsobrecupoa"),
									"Cupo Cliente Articulo Especial", JOptionPane.PLAIN_MESSAGE);
							return;

						}
					} else {

						JOptionPane.showMessageDialog(this, AppLocal.getIntString("message.cannotsobrecupob"),
								"Cupo Cliente", JOptionPane.PLAIN_MESSAGE);
						return;
					}
				} else {
					if (this.m_dSumArtEsp > 0) {
						...
						if (this.customerext.getCupArtEsp() >= this.m_dSumArtEsp) {
							...
							/* preguntar aqui */
							if (dSaldoCupo != 0) {
								/* si alcanza para cobrar en cupo cliente */
								m_aPaymentInfo.add(returnPayment);
								this.m_dSumArtEsp = 0;
								printState(-1);
								accepted = true;
						...
						VerificaRetornoSaldoCupo(returnPayment, dSaldoCupo, 1);
						if ((returnPayment.getName() == "prepago"))
						{
							this.SetFDian(false);
						}
				...
				if (this.customerext != null) {
					if ((returnPayment.getName() == "prepagosobrecupo")) {
						if (this.customerext.isSobreCupo()) {
							/* validar que tenga cupo articulo especial JDP */
							if (this.customerext.getCupArtEsp() == 0) {
								m_aPaymentInfo.add(returnPayment);
								...
								JOptionPane.showMessageDialog(this, AppLocal.getIntString("message.useprepagocupo"),
										"Prepago con Sobrecupo Cliente", JOptionPane.PLAIN_MESSAGE);
								return;
							}

						} else {
							JOptionPane.showMessageDialog(this, AppLocal.getIntString("message.cannotsobrecupo"),
									"Sobrecupo Cliente", JOptionPane.PLAIN_MESSAGE);
							return;
						}
					}
	...
	private javax.swing.JLabel m_jLblFDian;
	private javax.swing.JCheckBox m_jFDian;
