...
// FECHA        TAG      AUTOR             DESCRIPCION
// ---------------------------------------------------------------------------------------------------------
// 08/09/2017   JDP001   JOSE DE PAZ       Se agrega elemento recargaCupo a propiedades 
//   
...
import com.openbravo.pos.customers.RecargaCupoParamInfo;
...
public class JRootApp extends JPanel implements AppView {
	...
	private CustomerInfo m_dCustomerInfo;
	...
	public boolean initApp(AppProperties props) {
		...
		if (!AppLocal.APP_VERSION.equals(sDBVersion)) {
			...
			if (JRootApp.class.getResource(sScript) == null) {
				...
				JMessageDialog.showMessage(this,
						new MessageInf(MessageInf.SGN_DANGER, AppLocal.getIntString("message.noupdatescript")));
				m_appcnt.disconnect();
				return false;
			} else {
		...
		/* 25/06/2017 Realizar actualizaci�n de cupos */

		m_dlCustomers = (DataLogicCustomers) getBean("com.openbravo.pos.customers.DataLogicCustomersCreate");
		
		// JDP001 PROGRAMADA CUPOCLIENTE
		String recargaCupo = m_propsdb.getProperty("recargaCupo");	

		try {
			List<RecargaCupoParamInfo> recargaAutomatica = m_dlCustomers.getRecargaAutomaticaFecha();
			if (recargaAutomatica.size() == 1) {
				CustomerInfoExt customerext = new CustomerInfoExt("", "", "");
				RecargaCupoParamInfo oRecargaCupoParam = recargaAutomatica.get(0);
				customerext.setMaxdebt(oRecargaCupoParam.getM_dValor());

				switch (recargaCupo) {
				case "PROGRAMADA": {
					m_dlCustomers.updateCustomerMaxDebt(customerext);
					break;
				}
				case "CUPOCLIENTE": {
					m_dlCustomers.updateCustomerMaxDebt();
					break;
				}
				default: {
					m_dlCustomers.updateCustomerMaxDebt(customerext);
				...
				// Desactivar la recarga programada
				m_dlCustomers.updateRecargaCupoParam(oRecargaCupoParam.getM_sID());
	...
	public void setActiveCash(String sIndex, Date dStart, Date dEnd) {
		...
		if (m_propsdb.getProperty("recargaCupo")==null)
		{
			m_propsdb.setProperty("recargaCupo", "PROGRAMADA"); // JDP001			
		}
		m_dlSystem.setResourceAsProperties(m_props.getHost() + "/properties", m_propsdb);
	}
	...
	private void initComponents() {
		...
		setPreferredSize(new java.awt.Dimension(1055, 768)); /* JDP 17/06/2017 */
		...
		jLabel1.setText("<html><center>mdtPOS is a point of sale application designed for touch screens.<br>" 
